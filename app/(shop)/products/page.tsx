...
export default async function ProductsPage({
  ...
  if (searchParams.minPrice || searchParams.maxPrice) {
    where.price = {};
    if (searchParams.minPrice) { const min = parseFloat(searchParams.minPrice); if (!isNaN(min) && min >= 0 && min < 10000000) where.price.gte = min; }
    if (searchParams.maxPrice) { const max = parseFloat(searchParams.maxPrice); if (!isNaN(max) && max > 0 && max < 10000000) where.price.lte = max; }
  }
